# mote — 极轻量服务器监控探针（v2.2 设计大纲）

> 主控 `zk` + 被控 `bk`，类似 Komari / 哪吒，但更轻、更省心
> GitHub: `merlin-node/mote`
> 当前状态：v2.2 全功能已完成（含历史曲线、告警通知、备份/恢复、2FA、审计日志、主控自监控、节点 Tag、主题切换、公开只读页、移动端响应式）
...
设计原本规划三张表（`metrics_1s` / `metrics_1m` / `metrics_5m`），**列存已就位**，差一个后台聚合 goroutine。

**计划**：

- 后台 goroutine 定时聚合（每分钟一次）
- 1 小时内：秒级原始
- 24 小时内：1 分钟均值
- 30 天内：5 分钟均值
- 超过 30 天：自动清理
...
目前按原币显示，没做汇率折算。
**计划**：服务端每日拉一次 `open.er-api.com`，SQLite 缓存；展示模式 = 原币 / 折算 / 双显。
...
`billing_history` 表暂未创建。
**计划**：自动续费时归档一行（金额、周期起止、付款日）。
...
### 9.4 一键更新 `zk update` ❌ 未实现

**计划**：下载新二进制 → SHA256 校验 → 备份 → 替换 → 健康检查不通过自动回滚。
DB schema 只前进不回退，改动必须向后兼容（v2.1 已演练过：`ALTER TABLE ADD COLUMN` 平滑加列）。
...
- ICMP / TCP / HTTP(S) 三种探测类型
- 配置：目标、探测节点（多选）、间隔、保存时长、失败阈值
- 状态码 + 响应时间存到 SQLite
- 与告警系统打通：拨测失败 N 次触发通知
...
**只需要**：新主控能跑起来 + 被控能访问到新主控。
**不需要**：同域名 / 同 IP / 同端口 / 同操作系统。

### 14.2 三种场景

| 场景 | 方法 |
|---|---|
| A. 同域名换服务器 | 迁移数据 + 改 DNS，被控零改动 |
| B. 换域名/换 IP（核心差异化） | 迁移种子机制 |
| C. 仅保留节点信息 | 导入配置 + 重装被控 |

### 14.3 迁移种子机制（核心差异化，未做）

**思路**：旧主控关闭前，先给所有被控下发"未来的新地址"。被控本地存档，旧主控失联后自动连新地址。

**协议已留**：`MsgTypeMigration` 常量已在 `shared/protocol.go` 中定义，未实现 handler。

**计划数据结构**：

```toml
[migration]
new_url = "wss://new-panel.example.com"
new_token = "new_token_yyy"
fallback_after_seconds = 300
# 或 activate_after = "2026-06-01T00:00:00Z"
```

兜底：场景 C 已可用，用 `bk reconfig --server <新> --token <token>` 配合 ansible/pssh 批量。
...
| 类型 | 内容 | 用途 |
|---|---|---|
| 配置备份 | 节点元信息、账单、备注、告警规则、通知渠道 | 换主控、迁移、防误删 |
| 数据备份 | 配置 + 历史监控数据 | 完整快照、灾难恢复 |

### 15.2 计划格式

- **配置**：JSON，敏感字段（token、bot_token）用密码 PBKDF2 加密
- **完整**：tar.gz（含 data.db、config.json）

### 15.3 自动备份计划

- 频率：每天/每周/每月
- 保留：最近 N 个
- 存储：本地 / S3 / WebDAV（**S3/WebDAV 远程备份是关键差异化**，主控机器挂了备份还在云上）
...
目前数据全在 `/var/lib/zk/data.db`，手动 `cp` 也能备份。
...
Dockerfile 暂未提供。systemd 走通了。

### 16.5 反向代理（Nginx / Caddy） ✅ 文档

Nginx 模板已写在 README。Caddy 一行 `reverse_proxy 127.0.0.1:25774` 即可。
...
- ❌ 登录改成 Cookie/JWT，目前是 HTTP Basic（密码每次请求都发）
- ❌ 操作审计日志（谁在什么时候删了节点）
- ❌ 修改密码功能（目前要改 `/etc/zk/config.json` 然后重启）
- ❌ 2FA
...
- 看一台机器过去 24 小时的 CPU 曲线（列存就位，差降采样任务）
- 自动备份到 S3
- 一键更新到新版本
- 多货币换算（只看原币价格）
- 拨测外网延迟
